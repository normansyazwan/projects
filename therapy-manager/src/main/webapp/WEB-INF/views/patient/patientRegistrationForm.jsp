			...
			<form:form method="POST" commandName="patient"
				...
				<spring:bind path="bloodGroup">
					<div class="form-group ${status.error ? 'has-error' : ''}">
						...
						<div class="col-sm-6">
							<form:select path="bloodGroup" cssClass="form-control" id="bloodGroup" >
								<option value=""></option>
								<c:forEach var="bg" items="${bloodGroups}">
									<c:choose>
										<c:when test="${bg.enumValue eq bloodGroup}">
											<form:option  value="${bg.enumValue}">${bg.enumValue}</form:option>
										</c:when>
										<c:otherwise>
											<form:option value="${bg.enumValue}">${bg.enumValue}</form:option>
										</c:otherwise>
				...
				<spring:bind path="existingAilments">
					<div class="form-group ${status.error ? 'has-error' : ''}">
						<form:label path="existingAilments"
							cssClass="col-sm-2 control-label">Existing Ailments</form:label>
						<div class="col-sm-6">
					...
					<div class="form-group ${status.error ? 'has-error' : ''}">
						...
						<div class="col-sm-6">
							<form:input path="allergies" value="${allergies}"
								cssClass="form-control" placeholder="e.g. Dust, Antibiotics"
								maxlength="200" />
						</div>
						<div class="col-sm-4">
				...
				<div class="form-group">
					<form:label path="address.area" cssClass="col-sm-2 control-label">Locality</form:label>
					<div class="col-sm-6">
